...
function updateLearningProfile(
  ...
  profile.acquisitionRate = profile.acquisitionRate * (1 - LEARNING_RATE) 
...
function calculateCourseSize(course: Course): CourseSizeMetrics {
  ...
  const avgComplexity = course.modules
    .flatMap(m => m.lessons)
    .flatMap(l => l.informationPoints)
...
function predictCompletionTime(
  ...
  const personalizedHours = courseSize.estimatedHours 
...
function generatePredictionCurve(
  ...
  for (let hours = 0; hours <= maxHours; hours += maxHours / 100) {
    ...
    const mastery = 100 * (1 - Math.exp(-k * hours / courseSize.effectiveSize * 100));
...
type ProgressAlert = 
  ...
  | { type: 'behind'; daysBehind: number; message: string; suggestion: string }
  | { type: 'prediction_updated'; oldDate: Date; newDate: Date; reason: string };
```
