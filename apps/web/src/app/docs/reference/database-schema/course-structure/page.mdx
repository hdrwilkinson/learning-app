...
erDiagram
    ...
    User ||--o{ CourseRating : "writes reviews"
...
Stores individual user ratings and reviews for courses. Each review captures the reviewer's progress metrics (completion % and mastery %) at the time of review, providing transparency about their experience level. The Course model maintains denormalized `averageRating` and `ratingCount` fields for efficient queries.

**Key Features:**
- Only enrolled users can submit reviews
- Reviews display the reviewer's progress for transparency
- One review per user per course (can be updated)
...
| `completionPercent` | Float | Reviewer's course completion % at time of review (0-100) |
| `masteryPercent` | Float | Reviewer's average mastery % at time of review (0-100) |
| `createdAt` | DateTime | When the rating was created |
| `updatedAt` | DateTime | When the rating was last updated |

**Progress Snapshot:**

When a user submits a review, their current progress is captured:
- **Completion %**: Percentage of Information Points that have been introduced (state ≠ UNSEEN)
- **Mastery %**: Average mastery level across all IPs where the user has made progress

This provides context to other users reading the review—a 5-star review from someone at 80% completion/mastery carries different weight than one from someone at 5% completion.
