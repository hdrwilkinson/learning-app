...
interface QuizResponse {
  ...
  // === Context ===
  userId: string;
  questionId: string;           // FK to GeneratedQuestion (which has informationPointId)
  quizTypeId: string;
...
Determines which quiz type to use for a given IP. Quiz type availability is determined by:
1. The IP's available quiz types (stored in `InformationPointQuizType` junction table)
2. The user's current mastery level meeting the quiz type's unlock threshold

```
function selectQuizType(ip: InformationPoint, progress: InformationPointProgress): QuizType {
  // Get quiz types available for this IP from InformationPointQuizType junction table
  ipQuizTypes = getQuizTypesForIP(ip.id)  // Via InformationPointQuizType
  
  // Filter to active types where user meets mastery threshold
  available = ipQuizTypes
    .map(ipqt => getQuizType(ipqt.quizTypeId))
    .filter(qt => qt.isActive && progress.masteryLevel >= qt.minMasteryToUnlock)
  
  if (available.length == 0) {
    return getQuizType('binary')  // Fallback - always available
  }
...
**Note:** Quiz type availability per IP is defined at content generation time via the `InformationPointQuizType` junction table. The algorithm filters these at runtime based on the user's current mastery level.
...
function processSkipQuizResult(
  ...
  for (lesson in skippedLessons) {
    for (ip in lesson.informationPoints) {
      ...
      // Quiz types unlocked based on masteryLevel >= QuizType.minMasteryToUnlock
      // (evaluated at runtime via selectQuizType)
    }
  }
