...
| Comparison (Phase 2) | Gemini 2.5 Flash | Cross-IP questions requiring relationship understanding |
...
> **Schema Reference:** See `GeneratedQuestion` model in [`prisma/schema.prisma`](/prisma/schema.prisma)
...
### Question Buffer (Runtime Structure)

> **Note:** `QuestionBuffer` is a runtime/cache structure used by the question selection algorithm. It is not persisted in the database—buffer state is computed from `GeneratedQuestion` records and `QuizResponse` history.
...
### Question Usage (Runtime Structure)

> **Note:** `QuestionUsage` is computed at runtime from `QuizResponse` records. It is not a separate database table—question usage history is derived from the responses a user has given to each question.
...
async function onMasteryUpdate(
  ...
  const QA_GENERATION_THRESHOLD = 0.30;
  const QA_UNLOCK_THRESHOLD = 0.40;
...
function calculateNextEligible(usage: QuestionUsage): Date {
  ...
  const cooldownDays = COOLDOWN_CONFIG.initialCooldownDays 
...
### Comparison Question Generation (Phase 2)

> **Status: Phase 2** — Comparison questions are planned for future implementation. The schema fields (`secondaryIpId`, `comparisonType`, `ipPair`) are not yet in the database.
...
### Comparison Question Triggers (Phase 2)
...
async function checkComparisonEligibility(
  ...
  for (const relatedId of ip.relatedPoints) {
    ...
    if (relatedProgress.state === 'learning' || relatedProgress.state === 'mastered') {
  ...
  for (const prereqId of ip.prerequisites) {
    ...
    if (prereqProgress.state === 'learning' || prereqProgress.state === 'mastered') {
...
async function gradeQAResponse(
  ...
  const prompt = GRADING_PROMPT
    .replace('{question.questionText}', question.questionText)
    .replace('{question.keyPoints}', JSON.stringify(question.keyPoints))
    .replace('{question.acceptableVariations}', JSON.stringify(question.acceptableVariations))
    .replace('{question.commonMistakes}', JSON.stringify(question.commonMistakes))
...
async function processGenerationQueue(): Promise<void> {
  ...
  for (const [ipId, ipJobs] of Object.entries(groupedByIP)) {
    ...
    for (const job of ipJobs) {
      try {
        const questions = await generateQuestionBatch(ip, job.quizTypeId, job.count);
...
- Cooldown system
- Comparison questions (requires schema additions: `secondaryIpId`, `comparisonType`, `ipPair`)
...
const QUESTION_GENERATION_CONFIG = {
  ...
  // Thresholds
  qaGenerationMasteryThreshold: 0.30,
